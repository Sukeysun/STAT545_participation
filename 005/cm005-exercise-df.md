...
```r
select(gapminder, -lifeExp)          # stay inside the function, not store in the org dataframe
```
...
```r
a <- select(gapminder, -lifeExp)     # store the change in a variable "a"
b <- select(gapminder, -year, -pop)
```
...
```r
arrange(gapminder, lifeExp)   # frome low to high
```
...
```r
arrange(gapminder, desc(lifeExp)) # descending
```
...
```r
arrange(gapminder, year, lifeExp)  # first year then lifeExp
```
...
```r
## select(gapminder, year, lifeExp, country) %>% arrange(gapminder, year, lifeExp)

b <- gapminder %>% 
  select(year, lifeExp, country) %>%   # ctrl+shift+m
