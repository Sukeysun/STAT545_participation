...
```
method(formula, data, options)  **data should always be tidy data**
```
...
In R, takes the form `y ~ x1 + x2 + ... + xp` (use column names in your data frame). **'~' is euqal sign**
...
```{r}
tidy(my_lm)  ## p value for log(gdpPercap) is lower than 0.05 and thus a significant value
my_lm %>% 
