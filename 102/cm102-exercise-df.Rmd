...
```{r}
## search the string for particular pattern
...
```{r}
str_split(fox, pattern = " ")[[1]] %>%   ### the output is always a list
  length()
...
fruit %>% head
fruit %>% 
    str_pad(width=7, side="right", pad="$") %>%  ## at least 7
    head()
```
...
```{r}
str_length(fruit) ## the length of each entry
length(fruit)  ## how many entries in the vector
```
...
```{r}
str_c(words[1:4], words[5:8], sep=" & ")  ## 1&5 2&6..
str_c(words[3:4], words[5:8], sep=" & ")  ## the smaller one will be recycled
str_c(words[3:4], words[5:8], sep=" & ", collapse=", ")  ## seperate by ","
```
...
(num <- str_pad(number, width = 2, side= "left", pad = "0" ))
```
...
str_subset(countries, pattern = "T.i..n")
```
...
```{r}
str_view_all(countries, pattern = "i.a")  ## highlight where the "i.a" is
...
```{r}
str_view_all_match(countries, pattern = "[^aeiou][^aeiou][^aeiou]") ## ^: non
str_view_all_match(countries, pattern = "[aeiou][aeiou]")
str_view_all_match(countries, pattern = "[aeiou ][aeiou ][aeiou ]") ## spaces account
str_view_all_match(countries, pattern = "[^aeiou ][^aeiou ][^aeiou ]") ## spaces doesn't account
```
...
```{r}
str_view_all_match(countries, "ro+") ## at least one "o" follow "r"
```
...
```{r}
str_view_all_match(countries, "e{2}")  ### two "e"
```
...
```{r}
str_view_all_match(countries, pattern="a{2}|e{2}|ea|ae")
...
```{r}
str_view_all_match(countries, pattern = "land$")  ## "$"  end
str_view_all_match(countries, pattern = "$")
```
...
```{r}
str_view_all_match(countries, pattern = "^Ca")  ## ^: start with
```

Countries without a vowel? The word should start with a non-vowel, continue as a non-vowel, and end:

```{r}
str_view_all_match(countries, "^[^aeiouAEIOU]*$")  ## start with a non-vowel * 0 or more things before
```
...
str_view_all(c("abad", "abbd"), 
             pattern="(a)(b)\\1")  ## want to a follow by the first group so aba
...
## anything follow by anything then repeat the first group , sencond group 
