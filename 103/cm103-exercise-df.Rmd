...
- subset with `[`, `[[`, or `$`. [1]: the first [] of the list. [[1]][1] the first content of the first list
...
```{r}
map_chr(x, str_c, "potato.", sep="-")  ## str_c: jpin multiple strings into a single string
```
...
## use the head function instead
map_chr(wesanderson, head, n =1)

## map_dbl(wesanderson, head, n =1)
...
```{r}
map_chr(wesanderson, 1)  %>% unname()  ## this take off the name the vector
```
...
map(got_chars,`[`,desired_info) %>% 
  head()
map_dfr(got_chars,`[`,desired_info)  ## subselect desired_info and then convert to a dataframe
map_dfr(got_chars,head)
...
```{r}
##{} : only read the last one
got_chars %>% {
    
    tibble(
        name = map_chr(., "name"),  ## whatever is on the left side of %>% 
        born = map_chr(., "born")
    )
    map_dfr(.,head,n=1)
}
