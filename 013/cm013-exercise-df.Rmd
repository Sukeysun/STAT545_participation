...
p <- ggplot(mtcars, aes(hp, wt)) + 
    geom_point()
ggsave("E:/term3/sta 545/STAT545_participation/013/my_plot.png", p)
```
...
- Vector vs. raster: Images are stored on your computer as either _vector_ or _raster_.
    ...
    - __Vector__: represented as shapes and lines. `pdf`, 
...
p_scales +
    ...
    scale_colour_continuous(
        trans  = "log10", 
        breaks = 10^(1:10) ## how the number presents on the scale
    ) +
...
library(scales)
p_scales +
    scale_x_log10(labels=dollar_format()) + ##  x label is shown as $
    scale_colour_continuous(
        trans  = "log10",    ## legend's color is according to log10
        breaks = 10^(1:10),  ## ledgend is 10 100 1000....
        labels = comma_format() 
    ) +
...
ggplot(iris, aes(Sepal.Width, Sepal.Length)) +
    ...
    theme(axis.text = element_text(size = 13),
          strip.background = element_rect(fill = "orange"), #rectangle 
          panel.background = element_rect("blue"))
```
