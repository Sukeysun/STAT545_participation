...
gvsl + geom_point(shape = "$")  ## point is shown as "$"
```
...
```{r}
ggplot(gapminder, aes(lifeExp)) +
  facet_wrap(~ continent, scales = "free_x") +  ## the aes x dont need to be see for all these graphs
  geom_histogram(aes(fill = lifeExp > 60))
```
...
gvsl +geom_point(aes(size = pop, fill = continent), shape = 21,
                 colour = "black", alpha = 0.2)    # shape = 21 --circle 22 -square
```
...
ggplot(gapminder, aes(year, lifeExp)) +
  ...
  stat_smooth(method = "lm", aes(col = continent))
...
ggplot(gapminder, aes(year, pop)) +
  geom_line(aes(pop = gdpPercap, colour = country))
```
